...
export default function POSPage() {
  ...
  const applyVoucher = async () => {
    ...
    try {
      const { data } = await api.get(`/api/promotions/validate/${voucherCode}`, {
        params: { restaurantId: RESTAURANT_ID, orderAmount: subtotal }
      })
           ...
           {tables.map((t: any) => (
             <button 
              ...
              className={`aspect-square rounded-2xl flex flex-col items-center justify-center transition-all border-2 relative ${
                selectedTable?.id === t.id ? 'bg-blue-600 border-blue-600 text-white shadow-lg scale-105 z-10' : 
              ...
              <div className="space-y-3">
                 ...
                 {appliedPromotion && (
                   <div className="flex justify-between text-green-600 font-black text-xs uppercase tracking-widest">
                     <span>Giảm giá ({appliedPromotion.name})</span>
